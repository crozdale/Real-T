...
export default class Transactions extends Component {
  ...
  isValidRecipientAmountSet(index) {
    ...
    if (
      validateAmount(
        this.props.parseTokenAmount(value, false).toFixed(),
        MINIMUM_TRANSFER
      )
    ) {
  ...
  remainingBalance() {
    return new BigNumber(this.totalAmount).lte(this.props.balance)
      ? new BigNumber(this.props.balance).minus(this.totalAmount)
      : new BigNumber(0);
  }
  ...
  validateAmounts() {
    ...
    if (
      (this.state.recipientAmount || this.state.recipientAddress) &&
      !validateAmount(
        this.props
          .parseTokenAmount(this.state.recipientAmount, false)
          .toFixed(),
        MINIMUM_TRANSFER
      )
    ) {
                  ...
                  >
                    <Grid>
                      <Grid.Row>
                        ...
                        >
                          <Button
                            onClick={this.props.transferTokens}
                            disabled={
                              this.props.sendingTokens || !this.props.canSend
                            }
