...
function Row(props) {
  ...
  const invoiceSubtotal = row.totalAmount;
  const invoiceTaxes = 0.088 * invoiceSubtotal;
  const invoiceTotal = invoiceTaxes + invoiceSubtotal;
  const generatePurchaseType = (item) => {
                ...
                <TableBody>
                  {row.items.map((item) => (
                    <TableRow key={item.item._id}>
                      ...
                      <TableCell>{item.artOwner?.firstName} {item.artOwner?.surname}</TableCell>
                      <TableCell>{generatePurchaseType(item)}</TableCell>
