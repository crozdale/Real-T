...
router.post('/statistics', authenticated, isAdmin, async(req, res, next) => {
    try{
        ...
        const writers = await user_model.count({role: 'writer'});
        const readers = await user_model.count({role: 'reader'});
        const publishers = await user_model.count({role: 'publisher'});
