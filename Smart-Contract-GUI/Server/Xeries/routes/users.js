...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (role === 'promoter'){
            const user = await user_model.findOne({"username": username, "role": 'customer'});
            if (!user || !user?.purchased?.length) throw new Error("promoterErr");
        }
        ...
        if (e.message === 'promoterErr') res.sendStatus(423);
        // if user is already exist, Sending an error code of 421
        else if (e.code === 11000) res.status(421).json(e);
...
router.post('/cart', authenticated, async (req, res, next) => {
    try {
        await user_model.findOneAndUpdate({ _id: req.user }, { $push: { "cart": {item: req.body._id, purchaseType: 'original'} } }, {new: true});
        res.sendStatus(200);
    } catch (e){
...
router.get('/cartPopulated', authenticated, async (req, res, next) => {
    try {
        const user = await user_model.findOne({ _id: req.user }).populate({path:'cart.item', model: listingModel});
        res.json(user.cart);
    } catch {
        next("Internal server error: Can't get cart details");
    }
...
router.post('/checkout', authenticated, async (req, res, next) => {
    try {
        const { items, date, address, totalAmount } = req.body;
...
router.get('/myOrders', authenticated, async (req, res, next) => {
    try {
        const orders = await ordersModel.find({customer: req.user}).populate({path: 'items.item', model: listingModel}).populate({path: 'customer', model: user_model}).populate({path: 'items.artOwner', model: user_model})
        res.send(orders);
    } catch (error) {
