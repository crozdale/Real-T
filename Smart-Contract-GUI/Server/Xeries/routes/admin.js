...
router.post('/statistics', authenticated, isAdmin, async(req, res, next) => {
    try{
        ...
        const artOwners = await user_model.count({role: 'artOwner'});
        const customers = await user_model.count({role: 'customer'});
        const insurers = await user_model.count({role: 'insurer'});
